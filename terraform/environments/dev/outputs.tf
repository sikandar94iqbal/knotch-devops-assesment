...
output "argocd_url" {
  description = "ArgoCD URL - deterministic as soon as `apply` finishes, since it's just the hostname you set, not a runtime-assigned IP. Self-signed cert by default - your browser will warn, that's expected. Reachable only through this Gateway + the same Cloud Armor policy protecting the app; argocd-server has no public IP of its own."
  value       = "https://${var.argocd_hostname}/"
}

output "argocd_gateway_ip_hint" {
  description = "ArgoCD's Gateway gets its own external IP, separate from the app's - map var.argocd_hostname to this IP (/etc/hosts or curl --resolve) the same way you do for the app."
