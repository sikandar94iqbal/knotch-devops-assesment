...
# setup-dev.sh
#
# Walks through provisioning the `dev` environment end-to-end: Terraform
# apply -> ArgoCD reachable -> app deployed and reachable. Same shape as
# scripts/setup-prod.sh, just pointed at the dev project/cluster/hostnames.
# Every step that changes real infrastructure, writes a secret, or touches
# the shared git remote pauses first, prints what it's about to do and why
# it matters, and waits for an explicit "y".
#
# Prerequisites (not checked by this script):
#   - gcloud authenticated (`gcloud auth login`) with access to the dev project
#   - terraform (>= 1.5), kubectl, helm, git installed
#   - terraform/environments/dev/terraform.tfvars already has the right
#     project_id/hostname/certificate_mode (see README)
#   - terraform/bootstrap already run against the dev project (state
...
# --- Configuration - matches this repo's actual dev setup -------------------
PROJECT_ID="knotch-dev"
REGION="us-central1"
CLUSTER_NAME="tenant-dev-gke"
APP_NAME="knotch-demo-app-dev"       # ArgoCD Application name = Helm release name
APP_HOSTNAME="api-dev.tenant.internal"
ARGOCD_HOSTNAME="argocd-dev.tenant.internal"
THIRD_PARTY_SECRET_ID="tenant-dev-third-party-api-key"
...
VALUES_FILE="$REPO_DIR/helm/api-service/values-dev.yaml"
...
private GKE Autopilot cluster, a Cloud SQL Postgres instance (ZONAL,
db-f1-micro - dev sizing), a Cloud Armor policy, Certificate Manager
certificates (self-signed - no real domain needed), Secret Manager
secrets, and installs ArgoCD + External Secrets Operator into the cluster
via Helm. Takes roughly 10-12 minutes. Ongoing cost accrues from this
point on until the environment is torn down (dev is sized to be cheap,
not free)."
...
# --- Step 4: fill in values-dev.yaml -------------------------------------------

section "Step 4: Update helm/api-service/values-dev.yaml with real values"

confirm "Edits $VALUES_FILE in place, replacing placeholder tokens
(REPLACE_WITH_*) with the real values fetched above. Only touches these
known placeholder strings - nothing else in the file changes (and if
they're already filled in from a previous run, this is a harmless no-op).
Review the diff printed after this step before committing."
...
section "Step 6: Commit and push values-dev.yaml"
...
cd "$REPO_DIR"
git add helm/api-service/values-dev.yaml
git commit -m "Fill in dev Helm values from terraform output"
git push

# --- Step 7: connect repo to ArgoCD -------------------------------------------

section "Step 7: Connect this GitHub repo to dev's ArgoCD"
...
'argocd' namespace on the DEV cluster, containing your GitHub PAT. It is
...
confirm "Creates the 'app-of-apps' Application in ArgoCD on the DEV
cluster. ArgoCD will then discover argocd/dev/apps/api.yaml, create the
'$APP_NAME' Application, render the Helm chart, and create real
Deployment/Service/Gateway/HPA/PDB objects in the 'tenant-dev' namespace.
This is the last manual kubectl apply this environment ever needs -
everything after this is driven by git. Unlike prod, dev's sync policy
has prune: true, so future deletions from git actually remove resources
here automatically."

kubectl apply -f "$REPO_DIR/argocd/dev/app-of-apps.yaml"
...
wait_for "the app's own Gateway to get an external IP" 60 \
  bash -c "[ -n \"\$(kubectl get gateway $APP_GATEWAY_NAME -n tenant-dev -o jsonpath='{.status.addresses[0].value}' 2>/dev/null)\" ]"

APP_GATEWAY_IP="$(kubectl get gateway "$APP_GATEWAY_NAME" -n tenant-dev -o jsonpath='{.status.addresses[0].value}')"
...
Verify the private DB path itself (not just "the Pod started"):
