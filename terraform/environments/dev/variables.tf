variable "project_id" {
  description = "GCP project ID to deploy the dev environment into."
  type        = string
}
...
variable "db_tier" {
  description = "Cloud SQL machine tier. db-f1-micro is the smallest/cheapest tier - fine for dev traffic."
  type        = string
  default     = "db-f1-micro"
}
...
variable "third_party_api_key_secret_id" {
  ...
  type        = string
  default     = "tenant-dev-third-party-api-key"
}
...
variable "certificate_mode" {
  description = "\"managed\": real, DNS-validated Google-managed cert (needs a real domain). \"self_signed\": Terraform-generated self-signed cert - no domain required, useful before DNS is set up. See the certificate module for details."
  type        = string
  default     = "managed"
}
...
variable "argocd_hostname" {
  ...
  type        = string
  default     = "argocd-dev.tenant.internal"
}
